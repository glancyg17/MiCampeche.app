...
(async () => {
  ...
  try {
    ...
    //    mock of the supported ones. ──
    {
      ...
      // pushPref()/setPushPref() are pure localStorage helpers — testable
      // directly regardless of Push API support, same spirit as the
      // urlBase64ToUint8Array check above.
      window.localStorage.removeItem('mc_push_enabled');
      assert(window.pushPref() === true, 'with no stored preference yet, pushPref() defaults to true — push is opt-out, not opt-in');
      window.setPushPref(false);
      assert(window.pushPref() === false, 'setPushPref(false) is read back as false');
      window.setPushPref(true);
      assert(window.pushPref() === true, 'setPushPref(true) is read back as true — same key, not a one-way switch');
      window.setPushPref(false);
      assert(window.pushPref() === false, 'the same stored value is returned on a later, independent call, not recomputed from a live check — the literal point of this change');
      window.setPushPref(true); // restore the default for the rendering assertions below

      // MC is a top-level `const` in supabase-client.js, so — unlike the
      // plain `function`-declared names (openPreferences, setPushEnabled,
      // reconcilePushState, urlBase64ToUint8Array, …) — it never attaches
      // to `window` and isn't reachable from here directly.
      // hasPushSubscription()/subscribeToPush() are instead exercised for
      // real through the window-exposed functions that actually call
      // them, below.
      const settle = () => new Promise(r => setTimeout(r, 20));
      const savedSession = currentSession;

      // Signed-in account: Preferencias shows the push block (any
      // signed-in account now, not admin-only), settles to "not
      // supported" — real, since this environment has no Push API at
      // all — and the reconciler doesn't throw when called directly.
      // openPreferences() reads the cached lastFetchedAccount when one
      // already exists (same pattern as the real menu flow, where it was
      // just refreshed moments earlier) — refreshPendingBadge() is the
      // real function that keeps that cache honest after a session change
      // (doSignOut() itself calls it), so it's used here the same way
      // other tests already force a fresh re-fetch after reassigning
      // currentSession directly.
      currentSession = { user: { id: 'uid-1', is_anonymous: false, email: 'ricardo@example.com' } };
      await window.refreshPendingBadge();
      await window.openPreferences();
      await settle();
      assert(text('modal-body').includes('Notificaciones push'), 'a signed-in account sees the "Notificaciones push" block in Preferencias');
      assert(text('modal-body').includes('No disponibles en este navegador'), 'and it honestly reports unsupported in a browser without the Push API, rather than a fake on/off toggle');
      let reconcileThrew = false;
      try { await window.reconcilePushState(); } catch (_) { reconcileThrew = true; }
      assert(!reconcileThrew, 'calling the reconciler directly in an unsupported environment does not throw');
      window.closeModal();

      // Signed-out (anonymous) session: no trace of the feature at all —
      // the same "entirely absent, not merely hidden" gate Mensajes
      // already uses.
      currentSession = { user: { id: 'uid-1', is_anonymous: true, email: null } };
      await window.refreshPendingBadge();
      await window.openPreferences();
      await settle();
      assert(!text('modal-body').includes('Notificaciones push'), 'a signed-out session never sees the push-notifications block at all');
      window.closeModal();

      currentSession = savedSession; // restore for later tests
      await window.refreshPendingBadge(); // and re-sync the cached account so later tests aren't left on the anonymous one
    }
