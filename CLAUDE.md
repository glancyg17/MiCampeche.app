...
*Modal view stack (added 31 ago 2026):* the single `#modal-bg` hosts deeply nested views (Tu cuenta → Pendiente list → item review → motivo del rechazo). `mcModalStack` holds a re-render snapshot per level; "go deeper" calls `mcModalPushView(key)` first, and the ✕ / backdrop / hardware-back all call `mcModalBack()` — pop one level, or `mcModalBack('pendingList')` to jump back past intermediate levels after a terminal action (approve/reject). `closeModal()` still means "dismiss everything" (clears the stack) and is what the post-submit flows call. Single-view modals (post forms, product view) never push, so ✕ closes them outright as before.
