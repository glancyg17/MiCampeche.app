...
*Hardware back button (added 31 ago 2026):* on mobile (`isMobile()`) the Android system back button / gesture is wired through `window.history` so it peels one UI layer per press — nested modal view → open modal → open menu → open weather card → non-Inicio screen — and only exits the app from Inicio with nothing open. One synthetic `history.state.mcTrap` entry is kept alive whenever any dismissible layer shows (re-armed via a `MutationObserver` on the three `-bg` backdrops + `nav()`); `mcCloseTopLayer()` is the single peel action shared by the `popstate` handler and the in-app "‹" back-bars (`mcGoBack()`). `nav()` keeps a `mcScreenStack` breadcrumb: peer bottom-nav tabs are lateral (back from any returns to Inicio), sub-screens (details, "Cómo funciona", "Pagar servicios") stack. Dormant on desktop.

*Modal view stack (added 31 ago 2026):* the single `#modal-bg` hosts deeply nested views (Tu cuenta → Pendiente list → item review → motivo del rechazo; Tu cuenta → Mi negocio → editar negocio). `mcModalStack` holds a re-render snapshot per level; "go deeper" calls `mcModalPushView(key)` first, and the ✕ / backdrop / hardware-back all call `mcModalBack()` — pop one level, or `mcModalBack('pendingList')` / `mcModalBack('bizProfile')` to jump back past intermediate levels after a terminal action (approve/reject, or a business edit that returns to the profile). `closeModal()` still means "dismiss everything" (clears the stack) and is what the post-submit flows call. Single-view modals (post forms, product view) never push, so ✕ closes them outright as before. The account view's business section is one tappable box → `openBusinessProfile()` (full record + edit, edit reuses the `negocio_verificar` form and re-sends to review).

*Onboarding cards (added 31 ago 2026):* a pinned "cómo usar esta sección" card (white, amber accent, numbered step chips) at the top of each user-postable list (Eventos, Perdidos, Empleos, Avisos, Reportes) with steps + a CTA into the real post form; Alertas gets an explainer (no CTA — alerts are official-only). Rendered client-side by `onboardCard()` in each `render*` — NOT real DB rows. Dismissible per-device (`localStorage mc_onboard_<key>`); master switch `ONBOARDING_ENABLED` in `js/app.js` — flip to `false` (or delete) once the app is familiar.

*Contact / anonymity toggles (added 31 ago 2026):* Avisos, Perdidos and Empleos each have a "¿dejar un número para que te contacten?" seg toggle — the typed number is attached (and shown as a call button on the card) only when it's "sí" (`optContact()` in `supabase-client.js` gates it; `contact_info` columns already existed). Perdidos/Empleos default "sí" with the number pre-filled from the account; Avisos defaults "no" (safety posts often want no contact). Avisos also has a "¿cómo lo firmas?" toggle → `avisos.anonymous` (new col); when on, the card shows "Vecino anónimo" instead of the display name (admin's Pendiente view still shows the real submitter). Empleos `company` is now optional (nullable) for anonymous employers.
